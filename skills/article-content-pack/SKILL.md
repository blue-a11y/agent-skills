...
7. Write `cover.txt` as the cover-generation brief. The cover count and content must align one-to-one with the recommended title options in `publish.md`; create one cover prompt for each title option, with a minimum of 5 cover prompts. When a cover prompt includes a keyboard, make the keyboard a white Apple Magic Keyboard-style keyboard by default unless the user asks for another style.
